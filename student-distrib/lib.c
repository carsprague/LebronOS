    ...
    void putc(uint8_t c) {
        ...
        if(c == '\t')
        {
            ...
            for(i = 0; i<4; i++) // 4 is size of tab in spaces
            {
                ...
                *(uint8_t *)(video_mem + ((NUM_COLS * terminal_array[current_terminal].cursor_ypos + terminal_array[current_terminal].cursor_xpos) << 1) + 1) = ATTRIB;    
    ...
    void putc_for_keyboard(uint8_t c) {
        ...
        if(c == '\t')
        {
            ...
            for(i = 0; i<4; i++) // 4 is size of tab in spaces
            {
                ...
                *(uint8_t *)(video_mem_for_keyboard + ((NUM_COLS * terminal_array[terminal_shown].cursor_ypos + terminal_array[terminal_shown].cursor_xpos) << 1) + 1) = ATTRIB;    
